...
function getStr(sessionState,request){
    ...
    if (fromAnyAction(request,["Выход","Выйти"])){
        str = "Выхожу.."
        return [str,"выход",[{ title: 'Помощь', hide: true },{ title: 'Сюжетный режим', hide: true },{ title: 'Аркадный режим', hide: true }]]
    }
    if (fromAnyAction(request,["помощь","помогите","что ты умеешь","умения"])){
            ...
            if (quests==undefined){
                ...
                if (num2==-1){
                    if (fromAnyAction(request,["вопрос","потвтори","повторить","повтор"])){
                        ...
                        str += "Правильный ответ был "+quests[questionNum2]["correctAnswer"][0][0].replace("_"," ")+".\n";
                        questionNum+=1;
    ...
    return [str,paintID, [].concat(buttons,[{ title: 'Помощь', hide: true },{ title: 'Выход', hide: true }])];
}
...
function getAns(r, variants, arc = -1) {   
    let a = r["command"].split(/[!#$%&()*+,\-./:<=>?@^_`{|}№;]|\u005b|\u005d/).filter(Boolean);
    let str = "";
    let d = "";
    let ind = -1;
    ...
    variants.forEach(function(item, i, variants) {
        item.forEach(function(itemVar, i2, item) {
            if (itemVar.toLowerCase().indexOf("_") != -1){
                if (r["original_utterance"].indexOf(itemVar.replace(/_/gi, ' '))!=-1){
                    ind = i;
                }
            }
        })
...
const PORT = 3000;
server.listen(PORT, () => console.log("сервак запущен"));